...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distance_added = self.distance_added + self.get_distance_from(
            Point(x,y))
        self.x = x
        self.y = y
    ...
    def halfway_to(self,p1):
        x_value = (self.x + p1.x)/2
        y_value = (self.y + p1.y)/2
        return Point(x_value,y_value)
def run_test_init():
    """
    Tests the   __init__   metdod of the Point class.
...
def run_test_get_number_of_moves_made():
    ...
    """
    # ------------------------------------------------------------------
    # DONE: 8.  Follow the same instructions as in DONE above,
